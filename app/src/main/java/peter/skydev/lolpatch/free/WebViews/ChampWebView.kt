...
class ChampWebView : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (this.isNetworkConnected()) {
            val imageView = findViewById(R.id.gif) as ImageView
            Glide.with(this).load(R.drawable.loader).into(imageView)

            Thread {
                try {
                    var urlResponse =
                        URL("https://leagueoflegends.com/" + getURLWithLanguage(language) + "/news/champions-skins/champion-reveal/").readText()
                    val lastChamp =
                        urlResponse.split("href=\"/" + getURLWithLanguage(language) + "/news/champions-skins/champion-reveal/")[1].split(
                            "\""
                        )[0]

                    runOnUiThread {
                        this.webView.webViewClient = CustomWebViewClient(this)
                        this.webView.loadUrl("https://leagueoflegends.com/" + getURLWithLanguage(language) + "/news/champions-skins/champion-reveal/" + lastChamp)
                    }
                } catch (e: Exception) {
